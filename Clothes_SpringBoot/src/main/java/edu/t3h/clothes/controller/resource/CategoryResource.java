...
public class CategoryResource {
    ...
    public BaseResponse<?> searchCategoriesByCondition(@PathVariable String condition) {
        BaseResponse<List<CategoryDTO>> categoryDTO = categoryService.searchCategoriesCondition(condition);
        if (categoryDTO.getData() != null && !categoryDTO.getData().isEmpty()) {
            BaseResponse<List<CategoryDTO>> response = new BaseResponse<>();
            response.setCode(HttpStatus.OK.value());
            response.setMessage(Constant.HTTP_MESSAGE.SUCCESS);
            response.setData(categoryDTO.getData());
            return response;
        } else {
            return new BaseResponse<>(HttpStatus.BAD_REQUEST.value(), Constant.HTTP_MESSAGE.FAILED, null);
        }
