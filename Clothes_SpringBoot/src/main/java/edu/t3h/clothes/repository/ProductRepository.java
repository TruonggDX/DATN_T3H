...
public interface ProductRepository extends JpaRepository<ProductEntity, Long> {
    ...
    @Query("SELECT distinct p FROM ProductEntity p WHERE (p.name LIKE %:condition% OR p.code LIKE %:condition%) AND p.deleted = false")

    Page<ProductEntity> searchProductByNameAndCode(@Param("condition") String condition, Pageable pageable);
    ...
    @Query("SELECT p FROM ProductEntity p WHERE p.deleted=false AND (p.price BETWEEN :minPrice AND :maxPrice)")
    List<ProductEntity> findProductsInPriceRange(@Param("minPrice") double minPrice, @Param("maxPrice") double maxPrice);

    @Query("SELECT p FROM ProductEntity p WHERE p.createdDate >= :startDate  AND p.deleted=false ORDER BY p.createdDate ASC LIMIT 4")
    List<ProductEntity> listProductNew(@Param("startDate") LocalDateTime startDate);

    @Query(value = "SELECT COUNT(*) FROM ProductEntity WHERE deleted=false")
    Long countProduct();
